...
int main(int argc, char* argv[]) {
    ...
    printf("%6s | %-10s | %-2s | %-14s | %-2s | %-18s | %-3s | %-10s\n",
    ...
    while (fread(buffer, 16, 1, f) == 1) {
        ...
        if (op == OP_TLB_FILL) {
            printf("%6s | %-10s | %-2s | 0x%012lx | %-2u | %-18s | %-3u | 0x%08x\n",
                   "", get_op_str(op), "x", vaddr, vaddr_valid, "0x0", value_valid, tlb_paddr);
        } else {
            printf("%6s | %-10s | %-2u | 0x%012lx | %-2u | 0x%016lx | %-3u | %-10s\n",
                   access_str, get_op_str(op), id, vaddr, vaddr_valid, value, value_valid, "");
        }
