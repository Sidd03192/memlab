...
module tb_memsys_v2;
    ...
    localparam int PROGRESS_INTERVAL   = 500;
    ...
    //===== MPKI: previous MSHR state registers =====
    logic [1:0] prev_l1_mshr_state [0:1];
    logic [1:0] prev_l2_mshr_state [0:3];

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            prev_l1_mshr_state[0] <= 2'b00;
            prev_l1_mshr_state[1] <= 2'b00;
            prev_l2_mshr_state[0] <= 2'b00;
            prev_l2_mshr_state[1] <= 2'b00;
            prev_l2_mshr_state[2] <= 2'b00;
            prev_l2_mshr_state[3] <= 2'b00;
        end else begin
            prev_l1_mshr_state[0] <= dut.u_l1.mshr_state[0];
            prev_l1_mshr_state[1] <= dut.u_l1.mshr_state[1];
            prev_l2_mshr_state[0] <= dut.u_l2.mshr_state[0];
            prev_l2_mshr_state[1] <= dut.u_l2.mshr_state[1];
            prev_l2_mshr_state[2] <= dut.u_l2.mshr_state[2];
            prev_l2_mshr_state[3] <= dut.u_l2.mshr_state[3];
        end
    end

    //===== MPKI: L1 miss counter =====
    // Fires when any L1 MSHR transitions from IDLE to UNRESOLVED (new miss allocated)
    always @(posedge clk) begin
        if (rst_n) begin
            if (dut.u_l1.mshr_state[0] == 2'b01 && prev_l1_mshr_state[0] == 2'b00)
                l1_miss_cnt = l1_miss_cnt + 1;
            if (dut.u_l1.mshr_state[1] == 2'b01 && prev_l1_mshr_state[1] == 2'b00)
                l1_miss_cnt = l1_miss_cnt + 1;
        end
    end

    //===== MPKI: L2 miss counter =====
    // Fires when any L2 MSHR transitions from IDLE to UNRESOLVED (new miss allocated)
    always @(posedge clk) begin
        if (rst_n) begin
            if (dut.u_l2.mshr_state[0] == 2'b01 && prev_l2_mshr_state[0] == 2'b00)
                l2_miss_cnt = l2_miss_cnt + 1;
            if (dut.u_l2.mshr_state[1] == 2'b01 && prev_l2_mshr_state[1] == 2'b00)
                l2_miss_cnt = l2_miss_cnt + 1;
            if (dut.u_l2.mshr_state[2] == 2'b01 && prev_l2_mshr_state[2] == 2'b00)
                l2_miss_cnt = l2_miss_cnt + 1;
            if (dut.u_l2.mshr_state[3] == 2'b01 && prev_l2_mshr_state[3] == 2'b00)
                l2_miss_cnt = l2_miss_cnt + 1;
        end
    end
        ...
        begin
            ...
            case (op)
                ...
                OP_MEM_STORE: begin
                    ...
                    instruction_cnt = instruction_cnt + 1; // count memory instructions
                end
    ...
    initial begin
        ...
        begin : drive_trace
            ...
            while ($fread(trace_buffer, trace_fd) == 16) begin
                ...
                if (records_seen % PROGRESS_INTERVAL == 0)
                    $display("[%0d] records=%0d sent=%0d ops=%0d l1_misses=%0d l2_misses=%0d",
                             cycle_ctr, records_seen, records_sent, mem_ops_seen,
                             l1_miss_cnt, l2_miss_cnt);
        ...
        $display("----------------------------------------");
        $display("  instructions      = %0d (memory ops only)", instruction_cnt);
        $display("  l1_misses         = %0d", l1_miss_cnt);
        $display("  l2_misses         = %0d", l2_miss_cnt);
        if (instruction_cnt > 0) begin
            $display("  L1 MPKI           = %.2f", (l1_miss_cnt * 1000.0) / instruction_cnt);
            $display("  L2 MPKI           = %.2f", (l2_miss_cnt * 1000.0) / instruction_cnt);
        end else begin
            $display("  L1 MPKI           = N/A (no instructions counted)");
            $display("  L2 MPKI           = N/A (no instructions counted)");
        end
        $display("========================================");
    ...
    always @(posedge clk) begin
        if (rst_n && cycle_ctr > timeout_cycles) begin
            ...
            $display("  l1_misses=%0d l2_misses=%0d instructions=%0d",
                     l1_miss_cnt, l2_miss_cnt, instruction_cnt);
            if (instruction_cnt > 0) begin
                $display("  L1 MPKI=%.2f L2 MPKI=%.2f",
                         (l1_miss_cnt * 1000.0) / instruction_cnt,
                         (l2_miss_cnt * 1000.0) / instruction_cnt);
            end
